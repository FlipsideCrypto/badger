...
import { ActionTitle, Empty, TableSortHead } from '@components'
...
const ManagerTable = ({ badge, managers, canManage }) => {
	...
	const isTableHidden =
		!newManagers.length && (!managers || managers.length === 0)
	...
	const onAddNew = () => {
		setNewManagers([
			{ ethereum_address: '', isManager: true },
			...newManagers
		])
	}

	const onAddressChange = (e, index) => {
		setNewManagers(newManagers =>
			newManagers.map((manager, i) =>
				i === index
					? { ...manager, ethereum_address: e.target.value }
					: manager
			)
		)
	}
	...
	const onDelete = (index, isActive) => {
		if (isActive && isSelected(managers[index].ethereum_address))
			setManagersToRemove(managersToRemove =>
				managersToRemove.filter(
					manager =>
						manager.ethereum_address !==
						managers[index].ethereum_address
				)
			)
		else if (isActive)
			setManagersToRemove(managersToRemove => [
				...managersToRemove,
				{
					ethereum_address: managers[index].ethereum_address,
					isManager: false
				}
			])
		else
			setNewManagers(newManagers =>
				newManagers.filter((manager, i) => i !== index)
			)
	}

	const isSelected = managerAddress => {
		return managersToRemove.find(
			(manager, i) => manager.ethereum_address === managerAddress
		)
	}
	...
	const actions =
		newManagers.length + Object.entries(managersToRemove).length > 0
