...
import { ActionTitle, TableSortHead, Empty, InputAmountDelete, TableInputAddress } from '@components';
...
const HolderTable = ({ badge, canManage }) => {
    ...
    const { openHolderTransaction, isPrepared, isLoading } = useManageHolders({
        mints: [
            ...newHolders,
            ...Object.values(balanceChanges).filter((change) => change.pendingAmount > change.amount),
        ],
        revokes: Object.values(balanceChanges).filter(
            (change) => parseInt(change.pendingAmount) < parseInt(change.amount),
        ),
    ...
    const onAmountChange = (e, index, isActive) => {
        isActive
            ? setBalanceChanges((balanceChanges) => ({
                  ...balanceChanges,
                  [index]: { ...badge.users[index], pendingAmount: e.target.value },
              }))
            : setNewHolders((newHolders) =>
                  newHolders.map((holder, i) => (i === index ? { ...holder, pendingAmount: e.target.value } : holder)),
              );
    };

    const onAddressChange = (e, index) => {
        setNewHolders((newHolders) =>
            newHolders.map((holder, i) => (i === index ? { ...holder, ethereum_address: e.target.value } : holder)),
        );
    };
    ...
    const onDelete = (index, isActive) => {
        isActive
            ? setBalanceChanges((balanceChanges) => ({
                  ...balanceChanges,
                  [index]: { ...badge.users[index], pendingAmount: '0' },
              }))
    ...
    const actions =
        newHolders.length + Object.entries(balanceChanges).length > 0 ? [saveAction, addAction] : [addAction];
                                            ...
                                            <InputAmountDelete
                                                value={
                                                    balanceChanges[index]
                                                        ? balanceChanges[index].pendingAmount
                                                        : holder.amount
                                                }
                                                onChange={(e) => onAmountChange(e, index, true)}
                                                onDelete={() => onDelete(index, true)}
                                                isDeleting={
                                                    balanceChanges[index] && balanceChanges[index].pendingAmount === '0'
                                                }
                                                canManage={canManage}
                                            />
                                        </TableCell>
