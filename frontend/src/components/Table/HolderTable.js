...
const HolderTable = ({ badge, canManage }) => {
	...
	const { openHolderTransaction, isPrepared, isLoading } = useManageHolders({
		mints: [
			...newHolders,
			...Object.values(balanceChanges).filter(
				change => change.pendingAmount > change.amount
			)
		],
	...
	const onAddNew = () => {
		setNewHolders(newHolders => [
			{ ethereum_address: '', pendingAmount: '' },
			...newHolders
		])
	}
	...
	const onAmountChange = (e, index, isActive) => {
		isActive
			...
			: setNewHolders(newHolders =>
					newHolders.map((holder, i) =>
						i === index
							? { ...holder, pendingAmount: e.target.value }
							: holder
					)
			  )
	}
	...
	const onAddressChange = (e, index) => {
		setNewHolders(newHolders =>
			newHolders.map((holder, i) =>
				i === index
					? { ...holder, ethereum_address: e.target.value }
					: holder
			)
		)
	}
	...
	const onDelete = (index, isActive) => {
		isActive
			...
			: setNewHolders(newHolders =>
					newHolders.filter((holder, i) => i !== index)
			  )
	}
	...
	const actions =
		newHolders.length + Object.entries(balanceChanges).length > 0
