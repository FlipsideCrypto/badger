...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        if (!isMenuOpen) return () => window.removeEventListener('pointerdown', handleNavClick);

        window.addEventListener('pointerdown', (e) => handleNavClick(e));
    }, [isMenuOpen]);
    ...
    return (
        <div className={`navbar ${(isMenuOpen || isFilled) && 'filled'}`}>
            <div className="container">
                <Link to="/" className="logo"><LogoIcon /></Link>

                <div className={`links ${isMenuOpen && 'active'}`}>
                    <div className="navbar__links__left">
                        <Link to="/stories/">Customer Stories</Link>
                        <Link to="/faq/">FAQ</Link>
                        <a href={HOME_LINKS.gitbook} target="_blank" rel="noreferrer">
                            Docs
                        </a>
                    </div>
                    ...
                    <div className="navbar__links__right">
                        <Link className="enter" to="/dashboard/">
                            <button className="secondary">Enter app</button>
                        </Link>
                    </div>
                </div>
