...
const Metrics = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const response = await fetch("https://api.flipsidecrypto.com/api/v2/queries/63abf6c7-15b6-4a6b-8aed-bc450764abe0/data/latest")

            const data = await response.json()

            const dataKeys = data
                .filter(item => item.KPI in keys)
                .map(item => { return { [item.KPI]: item.Count } })
                .reduce((acc, curr) => { return { ...curr, ...acc } }, {})
