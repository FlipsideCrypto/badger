...
const BadgeFormContent = ({
	...
	const shouldUseHash =
		obj.image_hash && (!isEdit || (isEdit && name === badge?.name))
			...
			event: () =>
				openBadgeFormTransaction({
					...
					onSuccess: async ({ chain, receipt }) => {
						...
						navigate(
							`/dashboard/organization/${chainId}/${orgAddress}/badge/${id}/`
						)
					}
				})
		}
		...
		<>
			...
			<FormDrawer label="General" open={true}>
				<div className="badge__form__general">
					...
					<div
						className="form__group mobile__hidden"
...
const BadgeForm = ({ isEdit = false }) => {
	...
	const { address, organization, badges, badge, canManage, retrieve } =
