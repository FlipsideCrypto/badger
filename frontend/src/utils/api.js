...
async function postIPFSMetadata(props) {
	const { name, description, image, attributes } = props

	if (!name || !description || !image)
		return { error: 'Missing required fields for IPFS metadata upload' }
...
async function getBadgeImage(orgName, orgAddress, badgeId, badgeName) {
	...
	try {
		await fetch(url, {
			...
			.then(res => res.json())
			.then(data => {
				if (!data) throw new Error('Badge image could not be created.')
...
async function getPFPImage(char, address) {
	...
	try {
		await fetch(`${API_URL}/art/pfp/${encodedChar}/${encodedAddress}/`, {
			...
			.then(res => res.json())
			.then(data => {
				if (!data) throw new Error('PFP image could not be created.')
				response = getFileFromBase64(
					data.image,
					`generated_${encodedChar}_${encodedAddress}.svg`
				)
			})
			.catch(err => {
				throw new Error(err)
...
export { postIPFSImage, postIPFSMetadata, getBadgeImage, getPFPImage }
