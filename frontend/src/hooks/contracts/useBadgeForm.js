...
const getBadgeFormTxArgs = ({
	...
	if (functionName === 'setBadgeURI' && uriHash) args = [tokenId, uriHash]
	...
	if (functionName === 'multicall') {
		...
		const setBadgeURI = organization.abi.encodeFunctionData('setBadgeURI', [
			tokenId,
			uriHash
		])
		const setHook = organization.abi.encodeFunctionData('setHooks', [
			slot,
			[address],
			[true]
		])
		const configHook = organization.abi.encodeFunctionData('configHook', [
			slot,
			address,
			config
		])
	...
	const openBadgeFormTransaction = async ({
		...
		try {
			...
			transactionWindow.onStart({
				...
				body: `Please confirm the transaction in your wallet to ${
					isNew ? 'create' : 'edit'
				} your Badge.`,
				click: lastClick
			})
