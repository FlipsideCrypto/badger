...
const useOrgForm = obj => {
	...
	const functionName = obj.ethereum_address
		? 'setOrganizationURI'
		: 'createOrganization'
	...
	const openOrgFormTx = async ({
		...
		try {
			...
			transactionWindow.onSign({
				...
				body: `Badger hasn't detected your ${
					isCreate ? 'new Organization' : 'Organization changes'
				} yet. Please give us a few minutes to check the chain.`,
				hash: tx.hash
			})
			...
			receipt.events = receipt.logs
				.filter(log => log.address === Badger.address)
				.map(log => Badger.abi.parseLog(log))
