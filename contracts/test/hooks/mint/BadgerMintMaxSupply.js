...
describe("Badger", function () {
    ...
    async function deployNewHooksOrganization() {
        ...
        const Hook = await ethers.getContractFactory("BadgerMintMaxSupply");
    ...
    describe("BadgerMintMaxSupply.sol", async function () {
        ...
        it("revert: mint() > max supply", async function () {
            const { organization, owner, otherAccount } = await loadFixture(deployNewHooksOrganization);

            await organization.connect(owner).mint(otherAccount.address, 0, 500, "0x")

            await expect(organization.mint(otherAccount.address, 0, 501, "0x"))
                .to.be.revertedWith("BadgerMintMaxSupply::execute: Max supply exceeded.")
        });

        it("revert: configHook() supply > max", async function () {
            const { organization, hook, owner, otherAccount } = await loadFixture(deployNewHooksOrganization);

            const HOOK_SLOT = await organization.BEFORE_MINT();
            const hookConfig = await getEncodedHookConfig(hook, [0, 350]);

            await expect(organization.connect(owner).mint(otherAccount.address, 0, 500, "0x"))
                .to.emit(organization, "TransferSingle")
            ...
            await expect(organization.connect(owner).multicall(transactions))
                .to.be.revertedWith("BadgerMintMaxSupply::config: Max must be greater than the already minted supply.")
        });
    });
});