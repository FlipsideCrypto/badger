...
describe("Badger", function () {
    ...
    async function deployNewHooksOrganization() {
        ...
        const [,, blockedAccount] = await ethers.getSigners();
        const Hook = await ethers.getContractFactory("BadgerTransferBlocklist");
        hook = await Hook.deploy();
        hook = await hook.deployed();

        const HOOK_SLOT = await organization.BEFORE_TRANSFER();
        const hookConfig = await getEncodedHookConfig(hook, [blockedAccount.address, true]);
        ...
        return { hook, organization, owner, otherAccount, blockedAccount }
    }

    describe("BadgerTransferBlocklist.sol", async function () {
        it("revert: safeTransferFrom() transfer blocklist", async function () {
            const { organization, hook, blockedAccount, otherAccount, owner } = await loadFixture(deployNewHooksOrganization);
            ...
            await expect(organization.connect(otherAccount).safeTransferFrom(otherAccount.address, blockedAccount.address, 0, 1000, "0x"))
                .to.be.revertedWith("BadgerTransferBlocklist::execute: Invalid permission to transfer token.")
        });

        it("revert: safeTransferFrom() transfer blocklist for managers", async function () {
            const { organization, hook, blockedAccount, otherAccount, owner } = await loadFixture(deployNewHooksOrganization);

            await expect(organization.connect(owner).mint(blockedAccount.address, 0, 1000, "0x"))
                .to.be.revertedWith("BadgerTransferBlocklist::execute: Invalid permission to transfer token.")
        });
        ...
        it("call: safeTransferFrom() disabled hook", async function () {
            ...
            const HOOK_SLOT = await organization.BEFORE_TRANSFER();
            const hookConfig = await getEncodedHookConfig(hook, [blockedAccount.address, false]);
            ...
            await expect(organization.connect(otherAccount).safeTransferFrom(otherAccount.address, blockedAccount.address, 0, 1000, "0x"))
                .to.emit(organization, "TransferSingle").withArgs(otherAccount.address, otherAccount.address, blockedAccount.address, 0, 1000)
        });
    });
});